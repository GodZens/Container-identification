...
def otsu_threshold(src_img,debug):
    ...
    print(left_light)
    print(right_light)
    #如果左邊和右邊圖像差距大於10則分別處理，<則一起處理
    if(abs(left_light-right_light) > 10 ):
        if(left_light>right_light):
            left_unlight = unlight(left)
            Re_image = cv2.hconcat([left_unlight,right])
        else:
            right_unlight = unlight(right)                      
            Re_image = cv2.hconcat([left,right_unlight])
        if debug:
            display_image_cv2(left, "left")
            display_image_cv2(right, "right")
        
    else:
        Re_image = gray_img
    _, thresh_img1 = cv2.threshold(Re_image, 127, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
...
#     _, thresh_img1 = cv2.threshold(sharp_image, 127, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
...
#         display_image_cv2(clahe, "clahe")
#         display_image_cv2(blur_img, "blur_img")
#         display_image_cv2(sharp_image, "sharp_image")
